...
namespace Yea.Scheduler
{
    ...
    public static class TaskManager
    {
        ...
        private static readonly ConcurrentDictionary<Action, bool> RunningNonReentrantTasks =
            new ConcurrentDictionary<Action, bool>();

        private static readonly ConcurrentDictionary<Guid, Schedule> _runningSchedules =
            new ConcurrentDictionary<Guid, Schedule>();
        ...
        {
            get { return _runningSchedules.Values.ToArray(); }
        }
        ...
        public static Schedule GetSchedule(string name)
        {
            if (_tasks != null)
            {
                return _tasks.FirstOrDefault(x => String.Equals(x.Name, name, StringComparison.CurrentCulture));
            }
            return null;
        }
        ...
        private static void AddSchedules(IEnumerable<Schedule> schedules, ICollection<Schedule> immediateTasks,
                                         DateTime now)
        {
        ...
        public static void AddTask<T>(Action<Schedule> taskSchedule) where T : ITask, new()
        {
            if (taskSchedule == null)
                throw new ArgumentNullException("taskSchedule",
                                                "Please specify the task schedule to add to the task manager.");
